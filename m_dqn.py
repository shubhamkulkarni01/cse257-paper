...
class M_DQN(sb.DQN):
    def train(self, gradient_steps: int, batch_size: int = 100, entropy_tau = 0.30, alpha=1.0, lo=-3) -> None:
...
model = M_DQN("MlpPolicy", env, verbose=0, tensorboard_log=f'output/{env.spec.id}/', 
        learning_rate = lambda x: x * 1e-3 + (1-x) * 1e-4,
        buffer_size = 16000, tau=1, batch_size=512, target_update_interval = 8000, max_grad_norm=1,
        # exploration_fraction = 0.3,
        train_freq=1, learning_starts=3000, policy_kwargs={'net_arch': [512, 512]})
